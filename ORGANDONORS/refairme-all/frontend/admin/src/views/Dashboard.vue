...
export default {
  ...
  mounted: function () {
    var abaseurl = '/admin/api/referrals'
    this.$http.get(abaseurl).then(response => {
      this.refItems = response.body
    }, response => {
      alert('Error')
    })
    abaseurl = '/admin/api/jobs'
    this.$http.get(abaseurl).then(response => {
      this.tableItems = response.body
    }, response => {
      alert('Error')
    })
    abaseurl = '/admin/api/users'
    this.$http.get(abaseurl).then(response => {
      this.userItems = response.body
    }, response => {
      alert('Error')
    })
  },
